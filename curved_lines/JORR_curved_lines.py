...
def roi(img):
    ...
    if img is not None:
        ...
        shape = np.array(
            [[int(0), int(y)], [int(x), int(y)], [int(x), int(0.6 * y)], [int(0), int(0.6 * y)]])
    ...
    if lines is not None:
        for line in lines: # [260 304 261 328]
            for x1, y1, x2, y2 in line:
                ...
                if (slope > 0.5): # maybe 0.4 idk
                    cv2.line(img, (x1, y1), (x2, y2), rightColor, 10)
...
# use cv2.VideoCapture(0) to access camera, keep like this for pictures
while (cap.isOpened()):
    _, frame = cap.read()
    ...
    # path = r'C:\Users\NinJa\PycharmProjects\testCamera\curvedline1.png'

    # CURVED LINES
    # path = r'C:\Users\NinJa\PycharmProjects\testCamera\croppedcurvedline1.png'
