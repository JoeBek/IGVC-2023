...
def processImage(image):
    ...
    myline, direction = hough_lines(canny, 1, np.pi/180, 10, 20, 5)
...
def runAutonomousControls(zed):
    ...
    final, lineDirection = processImage(frame)  # process the current image and determine the best course of action according to the line detection


    # Control logic based on the best course of action given by object detection and line detection
    # TODO: Improve this control logic
    # Current logic: prioritize avoiding obstacles and only follow the lines if there are no obstacles

    autonomousDirection = depthSensorDirection

    if (depthSensorDirection == "up"): #If the line detection doesnt detect anything then turn based on the object detection rules
        autonomousDirection = lineDirection
        print("FOLLOWING LINE")


    return autonomousDirection
