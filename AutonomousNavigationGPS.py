...
DEPTH_VALUE = 400 # when objects are closer than DEPTH_VALUE, the robot will turn to avoid them
MAX_DEPTH = 400
ROBOT_SPEED = 30
...
def roi(img):
    ...
    if img is not None:
        x = int(img.shape[1])
        y = int(img.shape[0])
        shape = np.array([[int(0), int(y-100)], [int(x), int(y-100)], [int(x), int(0.5*y)], [int(0), int(0.5*y)]])
...
def comparePathToObstacles(coordinateList):
    ...
    if (angle > 34): # if a path cant be found go forward
