...
def comparePathToObstacles(coordinateList):
    ...
    while minDistanceRight < ROBOT_WIDTH/2 and minDistanceLeft < ROBOT_WIDTH/2 and angle < STOP_ANGLE:
    ...
    if (angle > STOP_ANGLE-1): # if a path cant be found go forward
        return 0
...
def runAutonomousControls(zed):
    ...
    leftSpeed = int(ROBOT_SPEED * (1 - (desiredAngle/STOP_ANGLE))) #POSSIBLE ERROR SPOT
