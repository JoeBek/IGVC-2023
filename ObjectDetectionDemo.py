...
ROBOT_WIDTH = 15 #Horizontal width of robot in cm
...
def roi(img):
    ...
    if img is not None:
        x = int(img.shape[1])
        y = int(img.shape[0])
        shape = np.array([[int(0), int(y-20)], [int(x), int(y-20)], [int(x), int(0.65*y)], [int(0), int(0.65*y)]])
...
def processImage(image):
    ...
    blur = cv2.GaussianBlur(grayscale(filterimg),(15,15),0)
