...
WAYPOINT_LATITUDE = 37.22156271
WAYPOINT_LONGITUDE = -80.42687393
...
def draw_lines(img, lines):
    ...
    try:
       ...
       #draw left line and everything to the right
        ...
        #leftLineStr = LineString([(left_line_x1,int(0.65*img.shape[0]),(left_line_x2, int(img.shape[0])))])
        #rightLineStr = LineString([(right_line_x1,int(0.65*img.shape[0]),(right_line_x2, int(img.shape[0])))])
        #if leftLineStr.intesects(rightLineStr):
...
def processImage(image):
    ...
    ret, thresholded = cv2.threshold(blur, 127, 255, cv2.THRESH_BINARY)

    canny = cv2.Canny(thresholded, 50, 120)
...
def comparePathToObstacles(coordinateList):
    ...
    while PATH_LENGTH > 0:
        ...
        if (angle < 34): # if a path cant be found try again with a shorter path length
